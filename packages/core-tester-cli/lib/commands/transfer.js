...
module.exports = class TransferCommand extends Command {
  ...
  async run(options) {
    ...
    if (!this.options.skipValidation) {
      logger.info(
        `Sender starting balance: ${Command.xphtoshiToXph(walletBalance)}`,
      )
    }
    ...
    if (!this.options.skipValidation) {
      logger.info(
        `Sender expected ending balance: ${Command.xphtoshiToXph(
    ...
    wallets.forEach((wallet, i) => {
      ...
      if (log) {
        logger.info(
          ...
          } (fee: ${Command.xphtoshiToXph(transaction.fee)})`,
        )
      }
    })
  ...
  async __sendTransactionsWithResults(runOptions, isSubsequentRun) {
    ...
    if (runOptions.primaryAddress && runOptions.expectedSenderBalance) {
      ...
      if (!walletBalance.isEqualTo(runOptions.expectedSenderBalance)) {
        successfulTest = false
        logger.error(
          `Sender balance incorrect: '${Command.xphtoshiToXph(
            walletBalance,
          )}' but should be '${Command.xphtoshiToXph(
      ...
      if (!balance.isEqualTo(runOptions.transactionAmount)) {
        ...
        logger.error(
          `Incorrect destination balance for ${
            wallet.address
          }. Should be '${Command.xphtoshiToXph(
            runOptions.transactionAmount,
          )}' but is '${Command.xphtoshiToXph(balance)}'`,
        )
      }
    }
