...
describe("Commands - Transfer", () => {
        ...
        expect(expectedTransactions).toEqual(
            expect.arrayContaining([
                expect.objectContaining({
                    vendorField: "foo bar",
                    amount: Transfer.__phantomToPhantomtoshi(expectedTransactionAmount),
                    fee: Transfer.__phantomToPhantomtoshi(expectedFee),
    ...
    it("should generate n transactions", async () => {
        ...
        const opts = {
            ...defaultOpts,
            amount: Transfer.__phantomToPhantomtoshi(2),
            transferFee: Transfer.__phantomToPhantomtoshi(0.1),
    ...
    it("should sign with 2nd passphrase if specified", async () => {
        const expectedTransactionAmount = Transfer.__phantomToPhantomtoshi(2);
        const expectedFee = Transfer.__phantomToPhantomtoshi(0.1);
        const opts = {
            ...defaultOpts,
            amount: expectedTransactionAmount,
            transferFee: expectedFee,
