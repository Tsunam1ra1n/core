    ...
    const { error, value } = Engine.validate(
        ...
        Engine.joi.object({
            id: Engine.joi
                ...
                .required(),
            // @ts-ignore
            blockid: Engine.joi.alternatives().try(Engine.joi.phantomBlockId(), Engine.joi.number().unsafe()),
            type: Engine.joi.number().valid(TransactionTypes.DelegateResignation),
